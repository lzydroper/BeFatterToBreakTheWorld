...
void gui::Button::setPositioin(float x, float y)
{
	this->shape.setPosition(sf::Vector2f(x, y));
	this->text.setPosition(
		this->shape.getPosition().x + (this->shape.getGlobalBounds().width / 2.f) - this->text.getGlobalBounds().width / 2.f,
...
//dropdownlist显示的不是当前，而是默认(为0)
//设置变量
gui::DropDownList::DropDownList(float x, float y, float width, float height, 
	sf::Font& font, std::string list[], 
	unsigned nrOfElements, unsigned default_index)
	: font(font), showList(false), keytimeMax(1.f), keytime(0.f),isChanged(false)
{
...
//Functions
void gui::ProgressBar::update(const int current_value, const int max_value, const int level)
{
	float percent = static_cast<float>(current_value) / static_cast<float>(max_value);

	this->back.setSize(
		sf::Vector2f(
			static_cast<float>(std::floor(maxWidth * ((level - 1) * 0.4f + 1))),
			this->back.getSize().y
		)
	);
	...
	this->inner.setSize(
		sf::Vector2f(
			static_cast<float>(std::floor(back.getSize().x * percent)),
