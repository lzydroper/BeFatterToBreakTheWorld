...
void Player::initVariables()
{
	...
	this->weapon = new Sword(1, 0, 0, 100, 20, "Resources/Images/Sprites/Player/sword.png");
...
{
	poison = new Poisoning;
	retard = new Retarding;
	...
	this->attributeComponent->reset = true;
	this->attributeComponent->updateStats(true);
	//this->attributeComponent->hp = 20;
...
const std::string Player::toStringCharacterTab() const
{
	...
	ss << "Level: " << ac->level << "\n"
		...
		<< "Weapon Damage Min: " << w->getDamageMin() + ac->damageMin << " (" << ac->damageMin << ")" << "\n"
		<< "Weapon Damage Max: " << w->getDamageMax() + ac->damageMax << " (" << ac->damageMax << ")" << "\n"

		<< "Strength: " << ac->liliang << "\n"
		<< "Agile   : " << ac->minjie << "\n"
		<< "Defense : " << ac->fangyu << "\n"
		<< "Luck    : " << ac->luck << "\n"
		<< "Length  : " << ac->length << " (times)\n";
		if (this->poison->times > 0 || this->retard->debuffTimerMax > 0)
		{
			ss << "DeBuff: \n";
			if (this->poison->times > 0)
			{
				ss << " - Poison : " << this->poison->times << " (times)\n";
			}
			if (this->retard->debuffTimerMax > 0)
			{
				int times = this->retard->debuffTimerMax - this->retard->debuffTimer.getElapsedTime().asMilliseconds();
				times /= 1000.f;
